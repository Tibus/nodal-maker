 ...
 * SVG (painter's algorithm, pure JS); 2D sketches → the drawing's own SVG.
 * Output: src/kernel/exampleThumbs.generated.ts
...
for (const f of readdirSync(dir).filter((x) => x.endsWith(".json")).sort()) {
  ...
  try {
    ...
    if (v.kind === "sketch2d") out[name] = sketchSVG(v);
    else { const m = meshOf(v); if (m) out[name] = isoSVG(m.vertices, m.indices); }
    if (out[name]) { ok++; process.stdout.write(`  ✓ ${name}\n`); }
    else { fail++; process.stdout.write(`  ∅ ${name} (no geometry)\n`); }
  } catch (e) {
...
const file =
  "// AUTO-GENERATED by scripts/gen-example-thumbs.ts — do not edit by hand.\n" +
  "// Small preview SVGs shown in the Simple-mode model picker.\n" +
  `export const EXAMPLE_THUMBS: Record<string, string> = ${JSON.stringify(out, null, 0)};\n`;
writeFileSync("src/kernel/exampleThumbs.generated.ts", file);
process.stdout.write(`\nwrote src/kernel/exampleThumbs.generated.ts (${ok} thumbs, ${fail} skipped, ${file.length} bytes)\n`);
