...
```
## Sketcher 2D contraint (Fusion-style) — `src/sketch/` + `SketchEditor.tsx`

Ajout d'un **éditeur d'esquisse paramétrique à contraintes**, façon Fusion 360.

### Architecture (cœur framework-free, partagé kernel ⇄ UI)

- **`src/sketch/model.ts`** — `SketchDoc` : `points` (x,y, `fixed?`), `entities`
  (`line | circle | arc | spline`, chacun `construction?`), `constraints`
  (géométriques + cotes drivantes nommées). Helpers : `mergePoints` (weld par
  partage d'id = snapping), `deleteEntity`, `dimensions`, `cloneDoc`.
- **`src/sketch/solver.ts`** — **Levenberg-Marquardt**, Jacobienne numérique,
  résolution linéaire dense. Contraintes : coincident, horizontal/vertical,
  parallel, perpendicular, equal, tangent, pointOn, midpoint, symmetric,
  concentric (via merge), fixed + cotes distance/distanceX/distanceY/radius/
  angle. **Régularisation faible** (W_REG) → stable en sous-contraint.
  `overrides` = valeurs de cotes injectées par les params du nœud. `pull` =
  drag mou (W_PULL < poids contrainte) → un croquis entièrement contraint reste
  **rigide**, seuls les DOF libres suivent le curseur. ~0.06 ms/solve.
- **`src/sketch/geometry.ts`** — tessellation en polylignes (rendu 2D/3D),
  `traceLoops` (boucles fermées, construction exclue), `bbox`.
- **`src/sketch/build.ts`** (seul module qui touche replicad) — doc résolu →
  `Drawing` (trous via cut/fuse selon containment).
- **`src/sketch/presets.ts`** — `starterRect()` (rectangle contraint de départ).

### Intégration graphe

- Nœud **`sketch`** (`nodes.ts`) : params `plane` + `doc` (kind `sketch`, édité
  via l'overlay) + **une cote drivante = un param éditable** (rendu dynamique
  dans `GeoNodeView`). Eval : lit le doc, applique les overrides de cotes,
  `solve`, `buildDrawing`. Output `sketch2d` (avec `plane`) → Extrude/Revolve.
- **Cotes ⇄ params du nœud** : changer une valeur (champ du nœud) re-résout et
  met à jour la 3D **live**. `commitSketch` (éditeur autoritaire à la sortie)
  mirroir doc→params ; ouverture : sync params→doc.
- **Plan honoré** : `sketch2d.plane` → `sketchOnPlane(plane)` dans extrude +
  preview (une esquisse XZ/YZ apparaît verticale). ⚠ les crits de sélection
  d'extrude restent en XY (limite connue pour un extrude non-XY).

### Éditeur (`SketchEditor.tsx`, overlay plein écran)

- Interaction via **`docRef` synchrone + `applyDoc()`** (mutate → solve →
  setState) : les handlers relisent les ids créés. Refs de chaîne pour les
  outils multi-clics.
- Outils : select/drag, line (auto H/V près des axes), rect, circle, arc
  3-points, spline (Catmull-Rom). Snapping points (weld) + grille adaptative.
- Contraintes sur sélection (barre d'icônes), cotes → panneau éditable.
- **Undo/redo** (⌘Z / ⇧⌘Z) : snapshots pré-édition, une seule par drag.
- **Construction** (touche X / ⋯) : entités de référence dashed, exclues du
  profil.
- Pan/zoom molette, glyphes de contraintes, labels de cotes, axes.
