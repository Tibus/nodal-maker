...
export const NODE_DESCRIPTIONS: Record<string, string> = {
  ...
  textOnFace: "Engrave or emboss text on a face (base plane + offset); needs a .ttf/.otf font.",
...
export const NODE_SPECS: Record<string, NodeSpec> = {
  ...
  textOnFace: {
    ...
    params: [
      { name: "text", kind: "text", label: "text", default: "AB" },
      { name: "size", kind: "number", label: "size", default: 10, min: 1, max: 200, step: 0.5 },
      { name: "font", kind: "font", label: "font (.ttf/.otf)" },
      { name: "plane", kind: "select", default: "XY", options: ["XY", "XZ", "YZ"] },
      { name: "offset", kind: "number", label: "face offset", default: 0, min: -300, max: 300, step: 0.5 },
      { name: "x", kind: "number", default: 0, min: -300, max: 300, step: 0.5 },
      { name: "y", kind: "number", default: 0, min: -300, max: 300, step: 0.5 },
      { name: "depth", kind: "number", default: 1, min: 0.1, max: 20, step: 0.1 },
      { name: "mode", kind: "select", default: "engrave", options: ["engrave", "emboss"] },
    ],
  },
  offset2d: {
    type: "offset2d",
  ...
  autoOrient: {
    ...
    output: "solid",
    params: [
      { name: "heightWeight", kind: "number", label: "height weight", default: 1, min: 0, max: 20, step: 0.5 },
    ],
  },
  supports: {
    type: "supports",
    label: "Supports (resin)",
    inputs: [{ name: "in", type: "solid" }],
    output: "solid",
    params: [
      { name: "angle", kind: "number", label: "overhang °", default: 45, min: 10, max: 80, step: 1 },
      { name: "spacing", kind: "number", label: "spacing", default: 5, min: 1, max: 30, step: 0.5 },
      { name: "pillarDia", kind: "number", label: "pillar Ø", default: 1.2, min: 0.3, max: 6, step: 0.1 },
      { name: "output", kind: "select", default: "model+supports", options: ["model+supports", "supports"] },
    ],
  },
  split: {
    type: "split",
